...
device_name = "STeaMi-R1"  # <- change en "STeaMi-R2" pour le 2e relay
...
async def ble_task():
    ...
    while True:
                ...
                if name and name.startswith("STeaMi") and name != device_name:
                    ...
                    if man_data and len(man_data) == 2:
                        ...
                        if name.startswith("STeaMi-S"):
                            # Si c’est un capteur, je relaie sa donnée
        ...
        if forwarded_distance is not None:
            ...
            if forwarded_distance > 0 and forwarded_distance < 300:
                man_data = struct.pack("b", 0)  # Commande LED OFF
                LED_RED.on()
            elif forwarded_distance >= 300 and forwarded_distance < 600:
                man_data = struct.pack("b", 1)  # Commande LED ON
                LED_GREEN.on()
            else:
                man_data = struct.pack("b", 2)  # Commande LED BLINK
                LED_BLUE.on()
